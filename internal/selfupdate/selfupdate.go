// Package selfupdate replaces the running uplink binary with the newest
// published release.
//
...
const Repo = "DustanBaker/uplink-composer"
...
// "uplink" or "uplink-app" (the windowless launcher).
...
// so `uplink-app` updates itself rather than pulling down the console build.
func programName() string {
	...
	if strings.HasPrefix(base, "uplink-app") {
		return "uplink-app"
	}
	return "uplink"
}
...
func Apply(ctx context.Context, rel *Release, progress func(done, total int64)) (string, error) {
	...
	tmp, err := os.CreateTemp(dir, ".uplink-update-*")
	if err != nil {
