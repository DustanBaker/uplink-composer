...
func FormFromRecipe(wsDir string, r *recipe.Recipe) (RecipeForm, error) {
	...
	if e.Family != Windows {
		...
		apps, err := ubuntuProgramsIn(filepath.Join(wsDir, filepath.FromSlash(r.Linux.Autoinstall.UserData)))
		if err != nil {
...
func ubuntuProgramsIn(path string) ([]string, error) {
	b, err := os.ReadFile(path)
	if err != nil {
		return nil, fmt.Errorf("its Ubuntu answers could not be read: %v", err)
	}
	text := string(b)
	...
	if !strings.Contains(text, "# Generated by DSKY.") {
		return nil, errors.New("its Ubuntu answers were written by hand")
	}
	var doc struct {
		Autoinstall struct {
	...
	if err := yaml.Unmarshal(b, &doc); err != nil {
		return nil, fmt.Errorf("its Ubuntu answers do not parse: %v", err)
	}
	...
	for _, a := range appcatalog.Catalog() {
		...
		if (u.Apt != "" && slices.Contains(doc.Autoinstall.Packages, u.Apt)) ||
			...
			(u.Flatpak != "" && flatpaks[u.Flatpak]) ||
			(u.Repo == appcatalog.RepoGoogleChrome && strings.Contains(script.String(), "google-chrome-stable")) {
			out = append(out, a.ID)
		}
	}
	return out, nil
