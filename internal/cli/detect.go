...
func cmdDetect(ctx context.Context, env *Env, args []string) error {
	...
	if !*resolve {
		...
		fmt.Println("  uplink install windows-11 --drivers")
		fmt.Println("Or see what the catalogs actually have, and cache it now:")
		fmt.Println("  uplink detect --resolve")
		return nil
	}

	e, ok := oscatalog.Get(*osID)
	if !ok {
		return fmt.Errorf("unknown OS %q — see `uplink catalog`", *osID)
	}
	...
	fmt.Println("\nThese are cached — `uplink install " + e.ID + " --drivers` will reuse them.")
	return nil
}
