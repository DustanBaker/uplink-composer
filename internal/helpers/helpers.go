...
func FindWimlib(helpersDir string) (string, error) {
	if env := os.Getenv("DSKY_WIMLIB"); env != "" {
		return env, nil
	}
...
// Check reports the tool situation on this host for `dsky doctor`.
